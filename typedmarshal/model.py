...
class MarshalModel:
    ...
    def _create_attr_info(cls, obj):
        if isinstance(obj, type):
            ...
            ins = obj
        attrs = [a for a in ins.__class__.__dict__ if not a.startswith('_')]
    ...
    def _is_builtin(typ) -> bool:
        if isinstance(typ, type):
            ...
            return typ.__class__.__name__ in __builtins__.__dict__
            ...
            if typ[0] is Any:
                ...
                for k, v in attr.items():
                    if not isinstance(k, typ[1]):
                        ...
                        dict_items.append(self._unmarshal_with_type_check(v, typ[2]))
                return dict_items
                ...
                if typ[2][0]() is not None:
                    raise TypeCheckError('Unmarshaller does not support arbitrary Union.')

                if attr is None:
                    return None
                else:
                    return self._unmarshal_with_type_check(attr, typ[1])


            elif typ[0] is Optional:
                if attr is None:
                    return None

                if not len(typ) > 0:
                    return attr
                return self._unmarshal_with_type_check(attr, typ[1][0])
